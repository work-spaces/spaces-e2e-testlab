...
def test_assert_path_not_exists(path, deps = []):
    run_add_exec(
...
def test_git_is_blobless_clone(path, deps = []):
    run_add_exec(
...
def test_git_is_shallow_clone(path, deps = []):
    run_add_exec(
...
def test_assert_file_contains(name, path, expected, deps = []):
...
def test_assert_file_not_contains(name, path, unexpected, deps = []):
